...
it("renders 5 stars by default", () => {
  const instance = renderer.create(<StarRating value={0} />).root;
...
it("renders star values of [1, 1, 0.5, 0, 0] for value of 2.5", () => {
  const instance = renderer.create(<StarRating value={2.5} />).root;
  const stars = instance.findAllByType(Star);
...
it("renders star values of [1, 1, 0.5, 0, 0, 0] for value of 2.5 and count 6", () => {
  const instance = renderer.create(<StarRating value={2.5} count={6} />).root;
  const stars = instance.findAllByType(Star);
